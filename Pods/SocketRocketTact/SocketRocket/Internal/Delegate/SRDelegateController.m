...
- (void)setDelegate:(id<SRWebSocketDelegate> _Nullable)delegate
{
    ...
    dispatch_barrier_async(self.accessQueue, ^{
        SRDelegateController *strongSelf = weakSelf;
...
- (void)setDispatchQueue:(dispatch_queue_t _Nullable)queue
{
    ...
    dispatch_barrier_async(self.accessQueue, ^{
        SRDelegateController *strongSelf = weakSelf;
        strongSelf.dispatchQueue = queue ?: dispatch_get_main_queue();
        strongSelf.operationQueue = nil;
    });
}
...
- (void)setOperationQueue:(NSOperationQueue *_Nullable)queue
{
    ...
    dispatch_barrier_async(self.accessQueue, ^{
        SRDelegateController *strongSelf = weakSelf;
        strongSelf.dispatchQueue = queue ? nil : dispatch_get_main_queue();
        strongSelf.operationQueue = queue;
    });
}
